...
function getReleaseConfig(): Configuration {
  return {
    ...
    protocols: [
      {
        name: "Catch URL Scheme",
        // scheme 统一小写，与 src/constants 的 defaultScheme 保持一致
        schemes: [(process.env.APP_NAME as string).toLowerCase()],
      },
    ],
    artifactName:
    ...
    mac: {
      ...
      extendInfo: {
        CFBundleURLTypes: [
          {
            CFBundleURLName: "Catch URL Scheme",
            // 与 src/constants 的 defaultScheme 一致：统一小写
            CFBundleURLSchemes: [
              (process.env.APP_NAME as string).toLowerCase(),
            ],
          },
        ],
      },
